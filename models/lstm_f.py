...
'''Code for the LSTM_f baseline. The "TEST_ON" variable indicates the year that
...
def data():
    ...
    trainX, trainY = ts[train_idx, 0:1, :], ts[train_idx, TEST_ON:TEST_ON+1, :]
    testX, testY = ts[test_idx, 0:1, :], ts[test_idx, TEST_ON:TEST_ON+1, :]
...
if __name__ == '__main__':
    ...
    best_run, best_model = optim.minimize(model=create_lstm_model,
                                              ...
                                              eval_space=True,
                                              trials=trials)    
    folder = '../results/results_lstm_f'
    if not os.path.exists(folder):
        os.mkdir(folder)
    pickle.dump(trials, open(folder+'/'+str(TEST_ON)+'trial2.p', "wb"))
