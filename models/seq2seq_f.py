...
'''Code for the seq2seq_f model. The "TEST_ON" variable indicates the year 
from which the prediction will take place, given the years falling within the
range [0:TEST_ON-1] (accepted values: [1-13]).'''
...
def data():
    ...
    trainX, trainY = ts[train_idx, 0:TEST_ON, :], ts[train_idx, TEST_ON:, :] 
    testX, testY = ts[test_idx, 0:TEST_ON, :], ts[test_idx, TEST_ON:, :] 
...
def create_lstm_model(trainX, trainY, testX, testY):
    ...
    model.add(LSTM({{choice([32,64,128,256,512])}}, input_shape=(trainX.shape[1],100), return_sequences=True))
    ...
    model.add(TimeDistributed(Dropout({{choice([0.1,0.25,0.5])}})))
    model.add(LSTM({{choice([32,64,128,256,512])}}, go_backwards=False, return_sequences=True))
    model.add(TimeDistributed(Dropout({{choice([0.1,0.25,0.5])}})))
    ...
    cosines = np.array([np.average(np.diag(cosine_similarity(preds[:,i,:], val_true[:,i,:]))) for i in range(trainY.shape[1])])
...
if __name__ == '__main__':
    ...
    best_run, best_model = optim.minimize(model=create_lstm_model,
                                              ...
                                              eval_space=True,
                                              trials=trials)
    folder = '../results/results_seq2seq_f'
