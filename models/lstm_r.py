...
'''Code for the LSTM_r baseline. The "TEST_ON" variable indicates the year that
will be reconstructed, along with the initial time point (accepted values: [1-13]).'''
...
def data():
    ...
    trainX = np.concatenate((ts[train_idx,0,:].reshape(-1,1,ts.shape[2]), ts[train_idx,TEST_ON,:].reshape(-1,1,ts.shape[2])), axis=1) 
    trainY = np.concatenate((ts[train_idx,0,:].reshape(-1,1,ts.shape[2]), ts[train_idx,TEST_ON,:].reshape(-1,1,ts.shape[2])), axis=1) 
    testX =  np.concatenate((ts[test_idx,0,:].reshape(-1,1,ts.shape[2]), ts[test_idx,TEST_ON,:].reshape(-1,1,ts.shape[2])), axis=1) 
    testY =  np.concatenate((ts[test_idx,0,:].reshape(-1,1,ts.shape[2]), ts[test_idx,TEST_ON,:].reshape(-1,1,ts.shape[2])), axis=1) 
    return trainX, trainY, testX, testY, ts
